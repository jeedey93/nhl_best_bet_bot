/**
 * Unified Yahoo Finance proxy.
 * Routes by ?action= param:
 *   action=price   &symbols=MFC.TO,ENB.TO  → stock prices
 *   action=search  &q=MFC                  → autocomplete results
 *   action=details &symbol=MFC.TO          → sector + dividend info
 */
...
const _priceCache   = new Map(); // key: sorted symbols → { ts, data }
const _searchCache  = new Map(); // key: query          → { ts, data }
const _detailsCache = new Map(); // key: symbol         → { ts, data }
...
async function fetchQuoteSummary(symbol) {
  ...
  return { sector: ap.sector || '', industry: ap.industry || '', annualDividend: sd.dividendRate?.raw ?? sd.trailingAnnualDividendRate?.raw ?? null, dividendFrequency: 'quarterly' };
}
...
async function fetchDividendEvents(symbol) {
  ...
  if (divs.length < 2) return { annualDividend: null, dividendFrequency: 'quarterly' };
