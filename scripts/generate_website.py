...
def format_dual_bet(raw_text):
    ...
    for pick in picks:
        ...
        md += f"<button class='share-button' onclick='handleShare(\"{pick_id}\", \"{sport_label}\")' id='share-btn-{pick_id}'>\n"
...
def extract_bet_of_day_from_prediction(content, sport_name, sport_emoji):
    ...
    html += f"<button class='share-button' onclick='handleShare(\"{pick_id}\", \"{sport_name}\")' id='share-btn-{pick_id}'>\n"
...
def update_latest_predictions(preliminary=False):
    ...
    content += " * Uses native Web Share API on mobile, falls back to screenshot download on desktop\n"
    content += " */\n"
    content += "async function handleShare(pickId, sport) {\n"
    content += "  const button = document.getElementById(`share-btn-${pickId}`);\n"
    content += "  const card = button.closest('.pick-card');\n"
    content += "  \n"
    content += "  if (!card) return;\n"
    content += "  \n"
    content += "  // Disable button while processing\n"
    content += "  button.disabled = true;\n"
    content += "  button.innerHTML = '⏳ Sharing...';\n"
    content += "  \n"
    content += "  try {\n"
    content += "    // Clone the card to manipulate it without affecting the page\n"
    content += "    const clonedCard = card.cloneNode(true);\n"
    content += "    \n"
    content += "    // Remove the share button from the clone\n"
    content += "    const shareBtn = clonedCard.querySelector('.share-button');\n"
    content += "    if (shareBtn) shareBtn.remove();\n"
    content += "    \n"
    content += "    // Create a temporary container for clean screenshot\n"
    content += "    const container = document.createElement('div');\n"
    content += "    container.style.position = 'absolute';\n"
    content += "    container.style.left = '-9999px';\n"
    content += "    container.style.background = 'white';\n"
    content += "    container.style.padding = '20px';\n"
    content += "    container.style.borderRadius = '12px';\n"
    content += "    container.style.width = '600px';\n"
    content += "    container.appendChild(clonedCard);\n"
    content += "    document.body.appendChild(container);\n"
    content += "    \n"
    content += "    // Simple share text with just the website\n"
    content += "    const shareText = `🎯 Parieur Discipliné\\nhttps://parieurdiscipline.com`;\n"
    content += "    \n"
    content += "    // Try native Web Share API first (works best on mobile)\n"
    content += "    if (navigator.share && navigator.canShare) {\n"
    content += "      try {\n"
    content += "        // Generate screenshot as blob\n"
    content += "        const canvas = await html2canvas(container, {\n"
    content += "          backgroundColor: '#ffffff',\n"
    content += "          scale: 2,\n"
    content += "          logging: false,\n"
    content += "          useCORS: true\n"
    content += "        });\n"
    content += "        \n"
    content += "        // Clean up temporary container\n"
    content += "        document.body.removeChild(container);\n"
    content += "        \n"
    content += "        const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png'));\n"
    content += "        const file = new File([blob], `parieur-discipline-pick.png`, { type: 'image/png' });\n"
    content += "        \n"
    content += "        // Check if we can share with file\n"
    content += "        const shareData = {\n"
    content += "          text: shareText,\n"
    content += "          files: [file]\n"
    content += "        };\n"
    content += "        \n"
    content += "        if (navigator.canShare(shareData)) {\n"
    content += "          await navigator.share(shareData);\n"
    content += "          button.innerHTML = '✓ Shared';\n"
    content += "        } else {\n"
    content += "          // Share text only if files not supported\n"
    content += "          await navigator.share({ text: shareText });\n"
    content += "          button.innerHTML = '✓ Shared';\n"
    content += "        }\n"
    content += "        \n"
    content += "        setTimeout(() => {\n"
    ...
    content += "          // User cancelled share\n"
    content += "          document.body.removeChild(container);\n"
    content += "          button.innerHTML = '<span style=\\'font-size: 1.5em;\\'>📤</span> <span>Share</span>';\n"
    content += "          button.disabled = false;\n"
    content += "          return;\n"
    content += "        }\n"
    content += "        // Fall through to screenshot download\n"
    content += "        console.log('Native share failed, falling back to download:', err);\n"
    content += "      }\n"
    content += "    }\n"
    content += "    \n"
    content += "    // Fallback: Generate and download screenshot\n"
    content += "    const canvas = await html2canvas(container, {\n"
    content += "      backgroundColor: '#ffffff',\n"
    content += "      scale: 2,\n"
    content += "      logging: false,\n"
    ...
    content += "    alert('Failed to share. Please try again.');\n"
    content += "    button.innerHTML = '<span style=\\'font-size: 1.5em;\\'>📤</span> <span>Share</span>';\n"
    content += "    button.disabled = false;\n"
    content += "  }\n"
    content += "}\n"
